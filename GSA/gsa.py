...
from transformers.models.llama.modeling_llama import LlamaAttention, rotate_half, apply_rotary_pos_emb
import argparse
from datasets import load_dataset
import functools
import json
# If you actually use these helpers in your repo, keep them:
...
from gsa_transformer import *   # assumes your helpers (e.g., sdpa_attention_forward) live here
...
class VanillaCache():
    ...
    def evict_internal(self):
        # TODO: implement if you ever enable Vanilla cache path during training.
        pass
...
class GSA_LlamaAttention(LlamaAttention):
    def __init__(
        ...
        # Recreate attention projections (same shapes as LlamaAttention)
        ...
        # ---- Register weights as buffers (device/dtype safe) ----
        pdev = next(self.parameters()).device if any(True for _ in self.parameters()) else torch.device("cpu")
        pdtype = self.q_proj.weight.dtype
        self.register_buffer("prefill_weights", prefill_weights.to(pdev, dtype=pdtype), persistent=False)
        self.register_buffer("decode_weights", decode_weights.to(pdev, dtype=pdtype), persistent=False)
...
if __name__ == "__main__":
    ...
    # For TinyLlama, you can switch to:
    ...
    parser.add_argument("--checkpoint", type=str, default="TinyLlama/GSA1_ResidualwithBlock_64/Ver_1/checkpoint-3000")
